...
echo -e "${BLUE}Fetching official Ubuntu mirrors...${NC}"

for country in "${COUNTRIES[@]}"; do
    url="https://mirrors.ubuntu.com/${country}.txt"

    while read -r mirror; do
        [[ -z "$mirror" ]] && continue
        [[ "$mirror" != */ubuntu* ]] && continue

        mirrors+=("${mirror%/}/ubuntu")
    done < <(curl -fs "$url")
done
...
if [[ ${#mirrors[@]} -eq 0 ]]; then
    echo -e "${RED}No mirrors retrieved from Ubuntu.${NC}"
    exit 1
fi
